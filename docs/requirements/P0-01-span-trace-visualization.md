...
> **状态**: 评审通过，待技术设计
> **优先级**: P0
> **参考**: 业界主流 Trace 调用链火焰图 / Trace Tree 可视化形态
...
| 维度 | AgentMonitor | Langfuse | LangSmith (OpenAI) |
|-----|--------------|----------|-------------------|
| **开源** | ✅ 完全开源 | ✅ 开源 | ❌ 闭源 |
| **自托管** | ✅ 轻量，SQLite/PostgreSQL | ✅ 需 Docker Compose | ❌ 仅云端 |
| **接入成本** | 一行 `autoInstrument`，支持 wrap | 需修改代码埋点 | 需使用 LangChain |
| **数据隐私** | ✅ 数据完全本地 | ✅ 自托管可本地 | ❌ 数据上传 OpenAI |
| **与评测联动** | ✅ 原生集成（同平台） | ❌ 需额外配置 | ❌ 仅 Trace |
| **Prompt 版本管理** | ✅ 原生集成 | ⚠️ 有限支持 | ⚠️ 有限支持 |
